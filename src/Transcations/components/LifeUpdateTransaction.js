...
export default function AlertDialog({ dailogProps : { open, message }, close}) {
    ...
    useEffect(() => {
        // console.log(history.push(), location.pathname);
    ...
    const onSubmit = async () => {
        ...
        setIsUpdating(true)
        axios.post(CrmforInsuranceService.CrmforInsuranceService.baseURL + '/api/transactions/life-transactions/update', values).then((response) => {
            setIsUpdating(false);
            close();
        }).catch((error) => {
            setIsUpdating(false)
                    ...
                    <div>
                        <div className="d-flex flex-column justify-content-center align-items-center">
                            ...
                            <div className="form-group col-12 m-auto py-2">
                                ...
                                <input  id="file" ref={imageRef1} name="application_form" type="file" accept="pdf" onChange={(e) => uploadFile(e)} />
                            </div>
                            <div className="form-group col-12 m-auto py-2">
                                <label className="h6">Policy Form</label><br />
                                <input id="file" ref={imageRef2} name="policy_form" type="file" accept="pdf" onChange={(e) => uploadFile(e)} />
                            </div>
                        </div>
                    </div>
